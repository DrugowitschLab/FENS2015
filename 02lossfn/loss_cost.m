function loss_cost(t, sig2)
%% plots the expected reward with accumulation cost over time
%
...
% Furthermore, it shows the expected reward over time in this setting. The
% expected reward is shown for three different costs, 0, 0.1, and 0.2.
%
...
cs = [0 0.1 0.2];
c_num = length(cs);
% cost colours, from black to red
c_cols = [linspace(0, 0.8, c_num)' zeros(c_num, 2)];
...
er = bsxfun(@minus, mean(pz1gx, 1), bsxfun(@times, ts, cs'));
...
legendstr = {};
for c_idx = 1:c_num
    plot(ts, er(c_idx, :), '-', 'LineWidth', 3, 'Color', c_cols(c_idx, :));
    legendstr = cat(2, legendstr, sprintf('cost %3.1f', cs(c_idx)));
end
% guides
plot(xlim, [0.5 0.5], 'k--', 'LineWidth', 1);
% labels and legends
